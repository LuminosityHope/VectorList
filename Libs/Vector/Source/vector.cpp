...
#include <utility>
    ...
    Vector::Vector<T>():data(nullptr),size(0),capacity(0){std::cout<<"Null"<<std::endl;}
    explicit Vector::Vector(const size_t sz)
    {
        size=sz;
        capacity=size;
        data=new T[capacity];
    }
    Vector::Vector<T>(const Vector& rhs)
    {
        size=rhs.size;
    ...
    Vector::Vector(const size_t sz,const T& value) {
        size=sz;
        capacity=size;
        data=new T[capacity];
        for (size_t i=0;i<size;i++) {
